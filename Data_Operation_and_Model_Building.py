...
class modelBuilding():
    ...
    def model_evaluation(self,model_name,n_jobs=-1):
        ...
        if model_name == "Xgboost":
            ...
            clf = LGBMClassifier(n_estimators=best_param["LightGBM"]["n_estimators"],max_depth=best_param["LightGBM"]["max_depth"],learning_rate=best_param["LightGBM"]["learning_rate"],n_jobs=n_jobs,verbose_eval=False)
            clf.fit(X_train,y_train)
            y_pred_test = clf.predict(X_test)
            y_pred_train = clf.predict(X_train)
